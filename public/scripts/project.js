...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(window.location.pathname)  // Загружаем JSON проекта
      .then(response => response.json())
      .then(data => {
          projectName.textContent = data.name;
          projectLocation.textContent = `Location: ${data.location}`;
          projectArea.textContent = `Area: ${data.area}`;
          projectProcess.textContent = `Process: ${data.process}`;
      })
